    ...
    for x in rent:
        ...
        if itemname not in li:
            if location is not None:
                if (int(price.text.replace('$', '').replace(',','')) <= 800)and (int(price.text.replace('$', '').replace(',','')) >= 10): #Keeps the apartments pulled in a range
                    ...
                    if str(posted.text) == today.strftime('%b '+ '%d'): #if the date of the posting is todays date it will notify your desktop
                        noti.notify(
                        title = itemname.text,
                        message = (linker['href'] +'\n' + price.text))
